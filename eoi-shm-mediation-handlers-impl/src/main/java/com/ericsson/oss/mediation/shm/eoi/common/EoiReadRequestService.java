...
public class EoiReadRequestService {

    private static final Logger LOGGER = LoggerFactory.getLogger(EoiReadRequestService.class);
    ...
    public Map<String, Object> readAttributes(final NetconfManager netconfManager, final String readFilter,
            final Map<String, Object> headers, final String moFdn) {
        ...
        try {
            ...
            LOGGER.error("Exception occurred in while processing for NetconfRequest: {}. Exception is : {}", readFilter,
                    ex);
    ...
    public Map<String, Object> prepareResponse(final NetconfResponse netconfFilteredResponse,
            final Map<String, Object> headers, final String moFdn, final List<String> requestAttributes)
        ...
        final Map<String, Object> additionalInfo = (Map<String, Object>) headers
                .get(ShmMediationConstants.ADDITIONAL_INFO);
        ...
        final List<String> primaryKeyAttributes = modelServiceHelper.getPrimaryKeyAttribute(moType, moNamespace,
                moMimVersion);
        if (primaryKeyAttributes == null || primaryKeyAttributes.isEmpty()) {
            LOGGER.error("In CommonReadRequestService.prepareAndSendResponse, {}",
                    ShmMediationConstants.NULL_KEY_RECEIVED_MSG);
        }
        if (requestAttributes.isEmpty()) {
            LOGGER.error("In CommonReadRequestService.ShmMediationConstants.NODE_HEALTH_CHECK_JOB_TYPE, {}",
                    ShmMediationConstants.JOB_TYPE);
            return new HashMap<>();
        } else {
            ...
            LOGGER.debug("Value of moName {}", moName);
            final Map<String, Object> responseAttributes = parseNetconfResponse(requestAttributes, netconfResponseData,
                    primaryKeyAttributes, moName, moType, multiKeyDelimiter);
            LOGGER.info("Response attributes: {} for MO FDN: {}", responseAttributes, moFdn);
            final Map<String, Object> newTransformedAttributes = modelTransformer.transformToDpsByData(moNamespace,
                    moMimVersion, moType, moName, responseAttributes, true,
    ...
    private Map<String, Object> parseNetconfResponse(final List<String> requestAttributes, final String responseData,
            final List<String> primaryKeyAttributes, final String moName, final String moType,
    ...
    public List<String> getRequestAttributesFromActionRequest(final Map<String, Object> headers) {
        ...
        final Map<String, Object> additionalAttributes = (Map<String, Object>) headers
                .get(ShmMediationConstants.ADDITIONAL_INFO);
